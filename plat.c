...
static void on_event(xcb_generic_event_t *ev)
{
    ...
    case XCB_KEY_RELEASE: {
        ...
        if (is_auto_repeat(kr))
            break;
        fb_key_set(map_key(XkbKeycodeToKeysym(dpy, kr->detail, 0, 0)), 0);
        break;
    }
...
int fb_main(void (*tick)(float dt))
{
    ...
    stage_w = FB_VIEW_W * SCALE;
    stage_h = FB_VIEW_H * SCALE;
