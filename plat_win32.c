...
{
    BITMAPINFO bmi;
    const Pixel *src = fb_back();
    int fb_w = fb_width();
    int ox = fb_fine_x();
    int oy = fb_fine_y();
    int x, y, vw = FB_VIEW_W, vh = FB_VIEW_H;

    for (y = 0; y < vh; y++) {
        const Pixel *row = src + (size_t)(oy + y) * fb_w + ox;
...
int fb_main(void (*tick)(float dt))
{
    ...
    dib_w = FB_VIEW_W * SCALE;
    dib_h = FB_VIEW_H * SCALE;
    dib = calloc((size_t)FB_VIEW_W * (size_t)FB_VIEW_H, sizeof(*dib));
    if (!dib) {
        fb_shutdown();
        return 1;
    }
    ...
    hwnd = CreateWindowExA(0, "XcbPixel", "xcb-pixel", style, CW_USEDEFAULT, CW_USEDEFAULT,
